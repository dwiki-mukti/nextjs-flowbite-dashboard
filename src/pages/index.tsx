...
import { HiBookOpen, HiEye, HiPencilAlt, HiPlus, HiTrash } from "react-icons/hi";
...
export default function IndexPage() {
   ...
   * Function Handler
   */
  function onShowDetailBook(dataRow: any) {
  ...
  function onDeleteBook(dataRow: any) {
    setBooks((prev) => prev.filter((newBook) => newBook.id != dataRow.id))
  }
  ...
  return (
    <>
            ...
            ((dataRow) => {
              return (
                <>
                  ...
                  <Button color="light" outline onClick={() => onShowDetailBook(dataRow)}>
                  ...
                  <Dropdown.Item
                    icon={HiEye}
                    onClick={() => onShowDetailBook(dataRow)}
                  >Detail</Dropdown.Item>
                  <Dropdown.Item
                    icon={HiTrash}
                    onClick={() => onDeleteBook(dataRow)}
      ...
      <Drawer
        open={StateFormBook.valuePrimaryKey != undefined}
        onClose={() => setStateFormBook({})}
        position='right'
      >
        <Drawer.Header
      ...
      <Drawer
        open={StateDetailBook.valuePrimaryKey != undefined}
        onClose={() => setStateDetailBook({})}
        position='right'
      >
        <Drawer.Header
          title={`DETAIL BUKU`}
...
IndexPage.getLayout = function getLayout(page: ReactNode) {
  return <EOfficePanelLayout sidebarActive='home'>{page}</EOfficePanelLayout>;
};
