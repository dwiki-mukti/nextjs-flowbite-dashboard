...
import { Dropdown, Flowbite, Pagination, Table as TableFlowbite } from 'flowbite-react';
import { HiDotsHorizontal, HiEye, HiPencilAlt, HiTrash } from "react-icons/hi";
            ...
            <div>
                <div className={`overflow-x-auto overflow-y-hidden ${className ?? ''}`}>
                    <TableFlowbite>
                        {!noHeader && (
                            <TableFlowbite.Head>
                                {!noNumber && <TableFlowbite.HeadCell style={{ width: "1px" }}>#</TableFlowbite.HeadCell>}
                                {prototypeTable?.map((column, indexColumn) => {
                                    return (
                                        <TableFlowbite.HeadCell key={indexColumn} className={column.className} style={column.style}>
                                            {column.title}
                        ...
                        <TableFlowbite.Body className="divide-y">
                            {((!DataTables?.dataRows?.length) || (DataTables?.statusCode == 202)) ? (
                                ...
                                DataTables.dataRows?.map((dataRow: Record<string, any>, indexDataRow) => {
                                    ...
                                    return (
                                        <TableFlowbite.Row key={indexDataRow}>
                                            {!noNumber && (
                                                <TableFlowbite.Cell>
                                            ...
                                            {prototypeTable?.map((column, indexColumn) => {
                                                ...
                                                return (
                                                    <TableFlowbite.Cell key={indexColumn}>
                                                        {typeof (keyData) == 'function' ? keyData(dataRow) : dataRow[keyData]}
                                                    </TableFlowbite.Cell>
                                                )
                                            })}
                                            <TableFlowbite.Cell className="inline-flex items-center gap-3 text-base">
                                                <Flowbite theme={{ theme: { button: { color: { light: 'border-none hover:bg-slate-100 hover:text-blue-500' } } } }}>
                                                    <Dropdown
                                                        label={<HiDotsHorizontal className='w-5 h-5' />}
